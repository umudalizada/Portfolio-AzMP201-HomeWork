...
const Wishlist = () => {
  ...
  let wish = useSelector((state) => state.cards.favorite);
 
  let user = allUser.find((user) => user.id === "2");
  if (!user) return null;

  const handleRemove = async (elem) => {
    const remove = user.favorite.filter((item) => item.id !== elem.id);
   
    await patchById(BASE_URL_USERS, user.id, { favorite: remove });
};
